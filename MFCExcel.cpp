...
long MFCExcelFile::GetA1LastRowCount()
{
	CRange range;
	CRange range1;
	range.AttachDispatch(excel_current_range_.get_Item(COleVariant((long)1), COleVariant((long)1)).pdispVal, true);
	range1.AttachDispatch(range.get_End(-4121));
	auto count = range1.get_Row();
	range.ReleaseDispatch();
	...
	for (index[0] = 0; index[0] < static_cast<long>(numRows); index[0]++)
	{
		...
		for (index[1] = 0; index[1] < static_cast<long>(numCols); index[1]++)
		{
			...
			if (tempRecur < static_cast<long>(vectorsize))
			{
				...
				//new_string.push_back(L"Hello World!");
			}
			else
			{
				// reuse temp1 here
...
bool MFCExcelFile::GetRangeCellString(DWORD iRowStart, DWORD iRowEnd, DWORD iColStart, DWORD iColEnd,std::vector<CString>& new_string)
{
	if (!iRowStart || !iRowEnd)
	{
		return false;
	...
	DWORD numCols = iColEnd - iColStart + 1;
	DWORD numRows = iRowEnd - iRowStart + 1;
	//new_string.resize(numCols*numRows);
	CString temp1, temp2;
	//temp1 will be reused as a blank string.
	temp1 = MFCExcelFile::GetCellName(iRowStart, iColStart);
	temp2 = MFCExcelFile::GetCellName(iRowEnd, iColEnd);
	...
	COleVariant vResult;
	vResult = range.get_Value2();
	saRet.Attach(vResult);
	long dim = saRet.GetDim();
	long LowerBd1, UpperBd1, LowerBd2, UpperBd2;
	saRet.GetLBound(1,&LowerBd1);
	saRet.GetUBound(1,&UpperBd1);
	saRet.GetLBound(2,& LowerBd2);
	saRet.GetUBound(2, &UpperBd2);
	//reuse temp1
#ifdef UNICODE
	temp1 = L"";
#else
	...
	long tempRecur;
	// Rows
	for (index[0] = LowerBd1; index[0] <=UpperBd1; index[0]++)
	{
		// Cols
		for (index[1] = LowerBd2; index[1] <=UpperBd2; index[1]++)
		{
			VARIANT val;
			//tempRecur = index[0] * (numCols)+index[1];
			//if (tempRecur < static_cast<long>(vectorsize))
			...
			//temp1.Empty();
			//CString  temp3(bstr);
			//new_string.push_back(temp1);
			new_string.push_back((CString)val.bstrVal);
			//SysFreeString(bstr);
		}
	}
